...
// Create a local function for showing toast messages to avoid circular imports
const logError = (title: string, description: string) => {
  console.error(`[Error] ${title}: ${description}`);
  // We'll import the toast in the calling code instead of here
...
export const invokeEdgeFunction = async <T = any>(functionName: string, payload?: any): Promise<T> => {
  try {
    ...
    if (!isSessionValid) {
      ...
      // Note: We're not calling toast directly here to avoid circular dependencies
    ...
    try {
      ...
      if (error) {
        ...
        if (typeof error === 'object' && error !== null) {
          ...
          // We'll handle toast notifications in the calling code
    ...
    // We don't call toast directly here to avoid circular dependencies
    throw error;
  }
};
